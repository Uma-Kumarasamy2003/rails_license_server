class LicensesController < ApplicationController
  # Skip CSRF verification for API-like actions
  skip_before_action :verify_authenticity_token, only: [:create_license, :start_trial, :activate_key, :validate_key, :extend_license]

  before_action :set_license, only: [:extend_license]

  # GET /
  def index
    render json: { message: "✅ License Server is running!" }
  end
  ...
  def create_license
    ...
    # Check if the generated key already exists (unlikely due to UUID but good practice)
  ...
  def start_trial
    ...
    # Check for existing trial for this device
    ...
    # Ensure the trial key is unique
  ...
  def activate_key
    ...
    # Check if the key is already associated with another device
    ...
    # Ensure no other license with this deviceId exists for a different key
    ...
    render json: { success: true, message: "Subscription key activated" }
  end
  ...
  def validate_key
    ...
    # Check if the key is tied to a different device
    if license.deviceId.present? && license.deviceId != device_id
    ...
    render json: { valid: true, message: license.license_type == "trial" ? "Trial active" : "Subscription active" }
  end
